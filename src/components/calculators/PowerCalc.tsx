...
export default function PowerCalc({ onResult }: { onResult: (r: string) => void }) {
  ...
  const calculate = () => {
    ...
    if (mode === 'vi' && !isNaN(v) && !isNaN(i)) {
      res = `P = ${(v * i).toPrecision(6)} W`;
    } else if (mode === 'ir' && !isNaN(i) && !isNaN(r)) {
      res = `P = ${(i * i * r).toPrecision(6)} W`;
    } else if (mode === 'vr' && !isNaN(v) && !isNaN(r) && r !== 0) {
      res = `P = ${((v * v) / r).toPrecision(6)} W`;
    } else {
  ...
  return (
    <div>
      <CalcSelect label="Formula" value={mode} onChange={setMode} options={[
        { value: 'vi', label: 'P = V × I' },
        { value: 'ir', label: 'P = I² × R' },
        { value: 'vr', label: 'P = V² / R' },
      ]} />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        {mode !== 'ir' && <CalcInput label="Voltage" unit="V" value={voltage} onChange={setVoltage} />}
        {mode !== 'vr' && <CalcInput label="Current" unit="A" value={current} onChange={setCurrent} />}
        {mode !== 'vi' && <CalcInput label="Resistance" unit="Ω" value={resistance} onChange={setResistance} />}
      </div>
