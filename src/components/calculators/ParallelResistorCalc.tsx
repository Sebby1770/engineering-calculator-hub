...
export default function ParallelResistorCalc({ onResult }: { onResult: (r: string) => void }) {
  ...
  const calculate = () => {
    ...
    const recipSum = vals.reduce((s, r) => s + 1 / r, 0);
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mt-4">
        <button onClick={addResistor} className="px-4 py-2 text-sm rounded-lg border border-surface-300 dark:border-surface-700 text-surface-600 dark:text-surface-400 hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors">
          + Add Resistor
        </button>
      ...
      {result && (
        <div className="mt-4 p-4 rounded-lg bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700">
          <span className="text-sm text-surface-500 dark:text-surface-400">Equivalent Resistance:</span>
